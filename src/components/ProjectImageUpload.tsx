...
export default function ProjectImageUpload({ projectId, currentImageUrl }: Props) {
  ...
  async function handleRemove() {
    if (!confirm('Remove the project image?')) return
    await fetch(`/api/projects/${projectId}/image`, { method: 'DELETE' })
    router.refresh()
  }
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ background: 'rgba(0,0,0,0.85)' }}>
          <div className="dark-modal rounded-xl shadow-2xl w-full max-w-lg p-6">
            ...
            <div className="flex gap-3">
              <button
                onClick={handleClose}
                className="dark-btn-outline flex-1 py-2 text-sm font-medium rounded-lg transition-all"
              >
                Cancel
              </button>
              <button
